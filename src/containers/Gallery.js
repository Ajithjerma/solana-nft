...
const Gallery = () => {
  ...
  const renderItem = (metadata, index) => {
    // console.log("mmmmmmmmmmmmmmm",metadata,index)
     axios.get(metadata.data.uri).then(res=>{
      console.log("response",res.data.collection.name);
    })
...
// console.log("ollectionCtx.collection",collectionCtx.collection)
  return (
    <Row style={{ margin: 60 }}>
      <Col span={24}>
